...
public final class CustomerBuilder {
    ...
    public Session login() {
        var response = new AuthClient().login(persona + "@shopkart.test", Secrets.get(persona + ".password")); response.then().statusCode(200);
        return new Session(response.path("token"), ((Number) response.path("customerId")).longValue(), persona);
    }

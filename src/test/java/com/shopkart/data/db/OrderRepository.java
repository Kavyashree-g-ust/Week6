...
public final class OrderRepository {
    ...
    public static long placedCount(long orderId, long customerId, int totalPaise) {
        ...
        try (var connection = DriverManager.getConnection(url, user, password);
            ...
            statement.setInt(3, totalPaise);
            try (var result = statement.executeQuery()) {
                result.next();
                return result.getLong(1);
            }
        } catch (Exception exception) {
            throw new IllegalStateException("Database assertion failed", exception);
        }
    }
    private static String databaseUrl() {
        String configured = System.getenv("SHOPKART_DB_URL");
        if (configured != null && !configured.isBlank()) return configured;
        // Fallback: use Testcontainers
        return MySqlSupport.start().getJdbcUrl();
    }

    private static String dbUser() {
        String configured = System.getenv("SHOPKART_DB_USER");
        if (configured != null && !configured.isBlank()) return configured;
        return MySqlSupport.start().getUsername();
    }

    private static String dbPassword() {
        String configured = System.getenv("SHOPKART_DB_PASSWORD");
        if (configured != null && !configured.isBlank()) return configured;
        return MySqlSupport.start().getPassword();
    }
